...
\texttt{-f}&\texttt{--no-78s}&
  Prohibit penultimate places above the treble\\
...
rotational and mirror symmetry.  \methsearch's handling of symmetry in
...
depending on whether the criteria can be satisfied.  The \verb+--limit+
...
\footnote{The reason for this choice of implementation is that a truly randomised
...
\methsearch\ supports six basic levels of falseness checking.  Five of these
can be specified by a single-letter argument to the \verb+-F+ option.  (The
sixth basic level is described in \sref{semicourse}.)
...
\subsection{Differentials and semicourses}\label{semicourse}
\index{semicourses|(}
...
However, in differential\index{differential} methods this is not generally 
true.  For example, 
Double Helix Differential Major\index{Double Helix Differential Major} 
