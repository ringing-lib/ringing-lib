...
options (\sref{misc_opt}) to require a row matching the given pattern to
appear in a particular place in at least one part, but without
specifying which part.  See \sref{pendrows} for more details.
...
the tenors running up forwards, then backwards four blows, then forwards
again in another four blows.  The leads heads of a regular caters method
are a group which can be specified on an \verb+-FP+ option as follows:
...
particular choice is quite subjective.
