...
\verb+--filter-lib+ option is given (\sref{misc_opt}), or if the \verb+$n+,
...
the first whitespace character on the line is an optional \textit{payload}
whose meaning (if any) is dependent on the context.\footnote{%
\methsearch\ considers the space, tab, carriage return and line feed
to be whitespace.  The payload begins at the first non-whitespace character
after the first instance of whitespace on the line.}\index{whitespace}

Optionally, the place notation may be preceded by the number of bells and a
colon, without spaces.  This is for compatibility with tools which can parse
method streams containing methods on varying stages.
