...
this is unnecessarily pedantic.)  The argument to the \verb+--require+
...
The places should be specified by bell symbol (see \sref{pn}).  If the lowest
place, $x$, is lead, it can be omitted; likewise, if the highest place, $y$,
is the back, it can be omitted.  Thus \verb+-Z-4+ is a traditional treble to
...
\fspec{COLUMNS} Many shells set this automatically.}
...
place notation first on the line followed by arbitrary content 
that is ignored.%
...
with either \verb+--filter+ or \verb+--filter-lib+.  It inverts the sense
...
The expression syntax (\sref{expr}) used by the \verb+--require+\loi{require} 
...
use this in conjunction with \verb+--filter+\loi{filter}.
...
\end{Verbatim}
This contains a single batch of \verb+-M+ options.  It scores four-bell runs
with an extra bonus for queens.%
...
In practice, however, such a circumlocution is unnecessary as the \textit{row}
argument to the \verb+-M+\textit{type}\verb+=+\textit{row} options is
actually a \textit{row expression} (\sref{rowexpr}).\index{row expression}  
This means that the group generator\index{group!generators} syntax can be used 
to write this example as just:
\begin{Verbatim}
...
This the unfortunate result that for many real-world searches, it will be
...
could be invoked by a \verb+--require+ option to check that a touch 
of the method is actually possible:
\begin{Verbatim}[commandchars=@~~]
--require='$(touchsearch -b6 -r -l720 @hfill\
               -Cb=4 -Cs=$[$12h eq "12" ? "234" : "456"] @hfill\
               -q --raw-count --limit=1 $p)'
\end{Verbatim}
...
\subsection{Efficency}\label{efficency}
...
Another efficency bottleneck can come from writing data to disk.  \methsearch\ 
...
from command invocations in row expressions, for example in \verb+-Fr+ options.
...
\index{lexicographic order}: \verb+12345678+, \verb+12346578+, 
...
printmethod -b8 -F "&-3-4-25-36-4-5-6-7,2" @hfill\
  | rowcalc -b8 "87654321 / <(-)" | musgrep -b8 1 
\end{Verbatim}
The final \texttt{musgrep} command is there to restrict the list of lead ends
...
\verb+--require+\loi{require} or \verb+-R+\oi{R} options.
A example of this was given in \sref{efficency} as an illustration
...
\begin{list}{\labelitemi}{\leftmargin=1em}
\item Method identifiers: \verb+$i+ (\sref{fmtstr}).
\item Support for reading \verb+$COLUMNS+ (\sref{output_opt}).
\item Fourths-place doubles lead ends now supported in \verb+$D+ 
  (\sref{fmtstr}).
