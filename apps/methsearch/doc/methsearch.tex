...
is entirely in upper case, then bell symbols are considered to be case
insensitive; if it is mixed case, or partly lower case, they are assumed
to be case sensitive.  The default alphabet is \textit{not} compatible with
...
can read them from a file which is then referenced on the command line by
...
Single quotes can be nested within double quotes, or vice versa; if a
...
\item The \verb+$BELL_SYMBOLS+ environment variable allows the bells used
  to be redefined.
\end{list}
