...
These may be basic patterns (\sref{muspattern}) or named patterns 
(\sref{musnamed}).  Customised music scores (\sref{musscore}) are not 
permitted.  The syntax for these is as given in chapter \ref{music},
...
(or no \verb+-R+ is given)
and none of the \verb+-q+, \verb+-o+, \verb+-O+, \verb+-H+, \verb+-C+, 
\verb+--raw-count+ or \verb+--node-count+ options are present.%
...
\texttt{-c}&\texttt{--count}&Count the number of touches found\\
  ...
  &Count the number of touches found, and print it without surrounding text\\
           &\texttt{--limit=N}&Limit the search to the first N touches\\
...
\texttt{gsiril} has already been mentioned in \sref{gsirilrow}.  As
mentioned there, it is also possible to use \texttt{gsiril} as a filter
to discover which methods are true to a particular composition.
