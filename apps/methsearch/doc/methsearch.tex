...
% Use an em-dash for bulleted lists
\usepackage{enumitem}
...
\newcommand{\ttcmdsee}[2]{\texttt{#1}\index{#1@{\texttt{#1}}|see{\texttt{#2}}}}
...
randomly (\sref{random}), filtering method streams (\sref{filtering}),
and other miscellaneous options (\sref{misc_opt}).  
...
The \verb+-b+ option is required except when filtering with a \verb+-I+ option
(\sref{filtering}) in which case it is optional.%
...
\verb+-U0+,\index{principles} or just \verb+-U+ as the argument to \verb+-U+
is optional and 0 is the default.
...
The combination \verb+-AU+ is useful when filtering as it avoids the need to
...
The \verb+--no-points+ and \verb+--has-points+ options\loid{no-points}
\loid{has-points} restrict the search to methods which do not or do have
points, fishtails and other such pieces of work where a pair of bells swap
consecutively an even number of times.  Methods like Thelwall Bob Minor or
Allendale Surprise Minor do have points even though the bells remain together
and later make a second pair of points, having the overall effect of a long
dodge.  The \verb+--no-unpaired-points+ and \verb+--has-unpaired-points+
\loid{no-unpaired-points}\loid{has-unpaired-points} options ignore paired sets
...
\begin{tabularx}{\textwidth}{llX}
\texttt{-r}&\texttt{--regular}&Require regular (Plain Bob or Grandsire) 
  lead head\\
&\texttt{--any-regular-lh}&Require any regular (Plain Bob or Grandsire)
  lead head, including those with extra hunts\\
...
\footnote{Until 2019, the Central Council refers to this form of
symmetry as \textit{double symmetry}\index{symmetry!double}.
Since their adoption of the new Framework for Method Ringing
(\url{https://cccbr.github.io/method_ringing_framework/}), 
they have used \textit{double symmetry} to refer to either glide or
mirror symmetry.  In common usage, \textit{double symmetry} is sometime
incorrectly reserved to mean the combination of glide and palindromic
symmetry.}
Methods with just glide symmetry are relatively rarely rung; 
...
The planes of symmetry (for \verb+-s+) are normally fixed at the
half-lead and lead-end, and or point of rotational symmetry (for
\verb+-k+) is normally fixed at the quarter and three-quarter leads for
principles (\verb+-U0+) or methods with an odd number of hunt bells
(e.g. \verb+-U1+); or are one blow later for methods with an even number
of hunt bells (e.g. \verb+-U2+).  The \verb+--floating-sym+%
\loid{floating-sym} allows to be at arbitrary points; however, it makes
the search \textit{much} slower as the requirement for floating symmetry
point is enforced by filtering the results of the search, instead of
pruning the search tree during the search.  This can be useful for
finding single- and twin-hunt methods in a single search, or for finding
principles with constraints on the lead head so that only certain
rotations will match.
...
as \verb+&-1-1-1,2+ or in full as:%
\footnote{These are the two variants produced by the \verb+$q+ and
\verb+$p+ variables described in \sref{variables}.}
\begin{quote}
 \verb-X.16.X.16.X.16.X.16.X.16.X.12-.
\end{quote}

Instead of using single-character bell symbols, a \textit{bell expression}% 
\index{bells!expression} can be used.  This is when the bell number is written
in decimal an enclosed in braces.  For example, the place notation for
Original Maximus can be written \verb+x1{12}+, where \verb+{12}+ is used
instead of the symbol \verb+T+.  A bell expression can also be written 
\verb+{N}+ or \verb+{N-+$n$\verb+}+, where $n$ is any integer.  In this case,
\verb+N+ denotes the number of bells, so \verb+x1{N}+ is the place notation
for Original on any stage.
...
&\texttt{--pn-format=FMT}&Modify the format used to print place notations\\
\texttt{-L}&\texttt{--library=LIB}&Use method library to look up method names\\
&\texttt{--unnamed-name=NAME}&Use \texttt{NAME} instead of
  `Untitled' for unnamed methods\loid{unnamed-name}\\
&\texttt{--named}&Only find methods in a provided library\\
&\texttt{--unnamed}&Only find methods not in a provided library\\
...
case \verb+-R$p\t$a+ is assumed, or unless \verb+--pn-format+ option is given,
in which case \verb+$q+ is used in place of \verb+$p+).
...
omitted \verb+-o+ option except that it doesn't clear the status line when
a method is found.  This is useful when standard output is redirected or
...
\url{http://methods.ringing.org/xml.html}.}
...
The \verb+--pn-format+ option\loid{fn-format} affects how place notation
\index{place notation} is printed by the \verb+$q+, \verb+$Q+, \verb+$U+ and
\verb+$V+ method variables (\sref{variables}).  Its argument is a
comma-separated list of strings which modify the format of the place notation
as follows:
\begin{itemize}
\item \ttcmdidx{dots} (which may be abbreviated as `\verb+.+') instructs
  \methsearch\ to always print a dot next to cross changes;
\item \ttcmdidx{external} (which may be abbreviated `\ttcmdsee{ext}{external}')
  instructs \methsearch\ to include all extenal places;
\item \ttcmdidx{lower-case} (or `\ttcmdsee{lcross}{lower-case}' or
  `\ttcmdsee{x}{lower-case}') instructs \methsearch\ to write the cross change
  as \verb+x+ instead of \verb+-+;
\item \ttcmdidx{upper-case} (or `\ttcmdsee{ucross}{upper-case}' or
  `\ttcmdsee{X}{upper-case}') instructs \methsearch\ to write the cross change
  as \verb+X+ instead of \verb+-+;
\item \ttcmdidx{std-symmetry} (or `\ttcmdsee{std-sym}{std-symmetry}') instructs
  \methsearch\ to only use the ampersand notation for palindromic place
  notations\index{symmetry!palindromic} when the symmetry point is the final
  change as in Plain Bob;
\item \ttcmdidx{no-symmetry} (or `\ttcmdsee{no-sym}{no-symmetry}') instructs
  \methsearch\ to never use the ampersand notation for palindromic place
  notations; and 
\item \ttcmdidx{plus-prefix} (or `\ttcmdsee{plus}{plus-prefix}' or `\verb-+-')
  instructs \methsearch\ to add a \verb-+- prefix to any block which does not
  have an \verb+&+ prefix.\footnote{This is useful if the result is going to be
  read by \texttt{gsiril} which requires all blocks of place notation to have
  one of these prefixes in order to distinguish them from integers.}
\end{itemize}
...
\subsection{Loading method libraries}\label{libload}
...
If no \verb+-L+ option is present, the \verb+$METHOD_LIBRARY+%
\fspecd{METHOD\_LIBRARY} \textitidx{environment variable}
may be used to specify one or more library.  Its value is a colon-separated
list of files to load.  How an environment variable is set depends 
...
\begin{Verbatim}
set METHOD_LIBRARY=C:\Documents and Settings\Richard\methods.xml
export METHOD_LIBRARY=/home/richard/methods.xml
setenv METHOD_LIBRARY /home/richard/methods.xml
\end{Verbatim}

Regardless of whether a library is loaded explicitly using the \verb+-L+
option or implicitly from the \verb+$METHOD_LIBRARY+ environment
variable, if its filename is not an absolute path (i.e.\ if it does not
...
\verb+$METHLIBPATH+\fspecd{METHLIBPATH} environment variable
is set to a colon-separated list of directories, these directories are 
search in turn for the file.%
\footnote{The two environment variables are synonymous.  If both are set, 
\verb+$METHLIBPATH+ is ignored.}

The \loid{named} and \loid{unnamed} options will restrict the search to
methods are either are or are not present one of the supplied method
libraries.\footnote{If the library includes unnamed methods, e.g. if it is a
complete list of all methods matching certain requirements, then
\verb+--named+ will find unnamed methods that are present in the library}

Libraries are only loaded if required.  This currently happens if one of the
\verb+--filter-lib+ ((\sref{filtering}), \verb+--named+ or \verb+--unnamed+
options are given, or if the \verb+$n+, \verb+$N+ or \verb+$i+ variables are
used (\sref{variables}).

\section{Over- and underworks}\label{overunder}

\begin{tabularx}{\textwidth}{lX}
\texttt{--overwork-map=FILE}&Use \texttt{FILE} as an overwork map 
                             to rewrite \verb+$V+\\
\texttt{--underwork-map=FILE}&Use \texttt{FILE} as an underwork map 
                             to rewrite \verb+$U+\\
\end{tabularx}

These two options take the name of a file which contains a mapping from over
or underwork place notations to names.  The format of the file is that of a
method stream (\sref{methstream}), and each method's payload is the name to
associate with that over- or underwork.

When an overwork or underwork is printed using the \verb+$V+ or \verb+$U+
\fspec{V}\fspec{U} method variables, the place notation for the over- or
underwork is generated and then compared with each method in the specified map
file in turn until one matches.  If method was found in the map file and its
payload is present, it is printed; otherwise the place notation is printed as
if formatted with \verb+$q+.

\methsearch\ can be used to help generate the over- and underwork map files by
running it with \verb+-qH'$V'+ to get a list of distinct overworks.  Save that
output to a file and append the name.  Alternatively, generate a list of all
methods using \verb+-R'$V\t$n'+ and delete all but one with each overwork.
...
The input format is a method stream.  This is described in detail in
\sref{methstream}, but is essentially a list of method place notations, one
per line.
Any text after the place notation is called the \textit{filter payload} 
and can be accessed by the \verb+$a+ method variable (\sref{variables}).
The output from \methsearch\ run without a \verb+-R+ option is suitable
input.\footnote{So is output from \methsearch\ run with a \verb+-R+ option
beginning \verb+$p+, \verb+$q+, \verb+$B:$p+ or \verb+$B:$q+.}
...
methsearch -b\textit{n} \textit{\rm\it[options]} > methods.txt
~ -- a comment to get vim's syntax highlighter back in sync:  $
\end{Verbatim}
...
Doing a search once and filtering it several times is frequently much faster
than repeating the search once per attempt to filter it.

The \verb+-b+ option\oi{b} is optional when filtering using \verb+-I+.  If it
is omitted, methods in the input stream should include the number of bells 
(e.g. by using a \verb+-R$B:$p+ option to \methsearch\ when creating the
stream).  In the absence of a \verb+-b+ option, any lines in the input stream
which do not specify the number of bells will be silently ignored.%
\footnote{This means they are also not found when \verb+--invert-filter+ is
used.}
...
efficiency, sometimes spectacularly when the equivalent option would
prune the search space.

The \verb+--row-match=+$n$\verb+=+\textit{pattern}\loid{row-match}
option takes two arguments separated by a \verb+=+.  It requires the
$n$th row of first lead of the method to match the specified music
pattern (\sref{muspattern}), which may be a simple pattern or a named
pattern (\sref{musnamed}) enclosed in \verb+<+\ldots\verb+>+.  In a
multi-part search the match may occur in any part (see \sref{pends}).
...
\def\Nno{\textit{[$m$[\verb+,+$n$]]}\/}
\def\F#1{\texttt{#1}\fspecd{#1}}
\begin{xltabular}{\textwidth}{lX}
\D\Nno\F{p}&The place notation (verbose format) --- see \sref{pn}\\
\D\Nno\F{q}&The place notation (concise format)\\
...
\D\F{V}&The place notation (or name) of the overwork\\
\D\F{U}&The place notation (or name) of the underwork\\
\D\No\F{s}&An index of the staticity of the method\\
\D\No\F{i}&The method identifier from the CC libraries\\
\D\No\F{a}&The filter payload --- see \sref{filtering}\\
...
The \verb+$q+ and \verb+$p+ variables may be prefixed by one or two integers.
If one number is given, then just the place notation of that one change is
printed, and if two integers are given, then the subsection of place notation
between the two specified changes is printed.\footnote{Thus
\verb+$+$n$\verb+p+ and \verb+$+$n$\verb+h+ are identical, but the $n$ may
only be omitted in the former.}
...
differential or differential hunter methods\index{differentials}: they
...
are assigned the name `Untitled'.\footnote{Unless this has been overridden 
with the \verb+--unnamed-name+ option.} In the case of Little Bob (and only
that case), the name, \verb+$n+, is the empty string; for all other methods,
...
\footnote{It is not present in the Central Council's XML libraries.}
...
behaviour.  In fact, this option is really a collection of related
sub-options, all pertaining to falseness, with the the first character after
the `\verb+F+' deciding which falseness option is being requested.

\begin{center}
\begin{tabular}{l@{\qquad}l@{\qquad}l}
Sub-option&  Summary& Section\\\hline
\verb+c+, \verb+l+, \verb+h+, \verb+n+, \verb+x+&
  Basic falseness& \sref{basic_false}\\
\verb+N+& Multi-extent methods& \sref{multiextent}\\
\verb+:+, \verb+CPS+& Falseness groups& \sref{falserest}\\
\verb+e+, \verb-e+-& Extent viability& \sref{extent}\\
\verb+r+, \verb+s+& Avoiding specific rows& \sref{avoidrow}\\
\verb+P+& Part end groups& \sref{pends}\\
\end{tabular}
\end{center}

The basic options governing whether the method should have 
...
Each level implies all the lower levels, and if several of these options are
used together, later ones will override the earlier ones.  The sole exception
is that \verb+-Fx+ may be in conjunction with any of the other, as explained
in \sref{multiextent}.
...
\subsection{Multi-extent methods}\label{multiextent}

The \verb+-FN+ option\oidF{N}{extents} is used to say how many times a row is
allowed to appear within the half-lead, lead or course (as specified with a
\verb+-Fh+, \verb+-Fl+ or \verb+-Fc+ option).  By default methods are proved
within a single extent, i.e.\ with \verb+-FN=1+.
Treble-dodging minimus methods are an example of when \verb+-FN=2+ is useful
in conjunction with \verb+-Fc+ to require the plain course is a true double
extent.\footnote{In fact, \texttt{-FN=2 -Fc} is unnecessary when searching for
methods with a palindromic three-lead course, as the symmetry guarantees the
course is a true double extent.  However it is useful in searches like
\texttt{-b4 -G1 -Fx -Fc -FN=2} to find non-palindromic methods whose plain
course is a true double extent.}\index{example!minimus, treble-dodging}

When rows are allowed to occur multiple times, it can make sense to use
\verb+-Fx+ as well as another option like \verb+-Fc+, because a repeated
change does not imply the method is false, when proved as a double extent.
Guilsfield Treble Bob Minimus\index{Guilsfield Treble Bob Minimus}, whose
place notation is \verb+&---14---34,12+ is an example of this.
...
use this in conjunction with \verb+-I+\oi{I} (\sref{filtering}).
...
The \verb+-FP+ option can also used with one or more \verb+--row-match+
\loi{row-match} options (\sref{misc_opt}) to require a row matching the
given pattern to appear in a particular place in at least one part, but
without specifying which part.  See \sref{pendrows} for more details.

\subsection{Methods which produce the extent}
...
$GM=S_n$.\footnote{Or more accurately, $\left| GM \right| = 
\left|G\right| \times \left|M\right| = n!$.}
...
Deane\index{Deane, Jonathan} and published in \textit{The Ringing World}
...
\begin{Verbatim}[commandchars=@~~]
methsearch -b8 -AU -n112 -sFh -FP="{23156487,13287654}"    @hfill\
  -Q"$o==15" -dkj -p2 --random --loop --timeout=1
\end{Verbatim}

The correct choice of timeout is critical to getting such a search to
...
\index{example!caters, plain with runs off front|(}
...
discussed in \sref{filtering}.  Secondly, it can output a list of methods
which is parsed by another program; an example of this was also given in
the discussion of filtering in \sref{filtering}.  The final ways are when 
...
one per line, with each line consisting of the following parts:%
...
\begin{itemize}
\item optionally, the number of bells followed by a colon;
\item the place notation of the method, in any recognised form (\sref{pn}),
written without whitespace;\footnote{\methsearch\ considers the space, tab,
carriage return and line feed to be whitespace.}\index{whitespace} and 
\item optionally, one or more whitespace character followed by further text
which is called the \textit{payload} and its meaning (if any) is dependent on
the context.\footnote{The payload begins at the first non-whitespace character
after the first instance of whitespace on the line.}
\end{itemize}

When invoked with the \verb+-I+ option (\sref{filtering}), 
\methsearch\ requires a method stream on standard input,\index{standard input}
and the payload is accessible wth \verb+$a+ method variable (\sref{variables}).
The \verb+--overwork-map+ and \verb+--underwork-map+ options (\sref{overunder}) 
take the name of a file whose contents are a method stream, and the payloads
are the names of the over- or underwork.

Frequently, \methsearch's output is a method stream — specifically whenever a
\verb+-R+ option starting with \verb+$p+, \verb+$q+, \verb+$U+ or \verb+$V+ is
given (which is the default when no \verb+-R+ is given), optionally prefixed
with a \verb+$B:+, and no statistical output or counts are requested.%
\footnote{This is because the \verb+-C+ and \verb+-H+ options also write to
standard output, and their output will not conform to the method stream
syntax, except in the case of \verb+-H"$U"+ or \verb+-H"$V"+.  A \verb+-u+
option is okay as the status is written to standard error rather than standard
output.\index{standard output}\index{standard error}}

The \verb+touchsearch+ (\sref{touchsearch}) 
and \verb+gsiril+ (\sref{gsirilmeth}) programs, 
as well as \methsearch\ itself when filtering (\sref{filtering}),
...
\item Improved falseness: \verb+-Fs+ (\sref{avoidrow}), 
  \verb+-FP+ (\sref{pends}).
\item Library filtering: \verb+--filter-lib+ (\sref{filtering}).
...
\subsection{During 2010 \& '11}

\begin{list}{\labelitemi}{\leftmargin=1em\itemsep=0em}
\item Method identifiers: \verb+$i+ (\sref{variables}).
\item Filter payloads: \verb+$a+ (\sref{filtering} and \sref{variables}). 
\item Filter inverting: \verb+--invert-filter+ (\sref{filtering}).
...
\item Abbreviations for the \verb+--require+ and \verb+--filter+ options:
  \verb+-Q+ and \verb+-I+, respectively (\sref{filtering}, \sref{misc_opt}). 
...
\subsection{During 2020}

I did little or no development on \methsearch\ between 2012 and 2019.  The
program did most of what I wanted, and the major features I still would have
liked to add -- namely support for alliance methods\index{alliance methods}
and improved support for principles\index{principles} and
differentials\index{differentials} -- all seemed larger projects than I had
time to do.  In 2020 I returned to the code and have added several relatively
small new features.
...
\item Loading libraries implicitly: \verb+METHOD_LIBRARY+ (\sref{libload}).
\end{list}

\subsection{During 2021}

\begin{list}{\labelitemi}{\leftmargin=1em\itemsep=0em}
\item Multi-extent methods: \verb+-FN+ (\sref{multiextent}).
\item Over and underworks: \verb+$V+, \verb+$U+ (\sref{variables});
  \verb+--overwork-map+, \verb+--underwork-map+ (\sref{overunder}).
\item Changing name used for unnamed methods: \verb+--unnamed-name+ 
  (\sref{output_opt}).
\item Customised formatting of place notation: \verb+--pn-format+ 
  (\sref{output_opt}).
\end{list}

\subsection{During 2022}

\begin{list}{\labelitemi}{\leftmargin=1em\itemsep=0em}
\item Named and unnamed methods: \verb+--named+, \verb+--unnamed+ 
  (\sref{output_opt}).
\item Bell expressions in \verb+-m+ (\sref{pn}).
\item Requiring and excluding points: \verb+--no-points+, \verb+--has-points+,
  \verb+--no-unpaired-points+, \verb+--has-unpaired-points+
  (\sref{int_struct}).
\end{list}
