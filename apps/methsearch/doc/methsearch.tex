...
as follows:
\begin{itemize}
\item \verb+dots+ (which may be abbreviated as `\verb+.+') instructs
  \methsearch\ to always print a dot next to cross changes;
\item \verb+external+ (which may be abbreviated `\verb+ext+') instructs
  \methsearch\ to include all extenal places;
\item \verb+lower-case+ (which may be written `\verb+lcross+' or
  `\verb+x+') instructs \methsearch\ to write the cross change as \verb+x+ 
  instead of \verb+-+;
\item \verb+upper-case+ (which may be written `\verb+ucross+' or
  `\verb+X+') instructs \methsearch\ to write the cross change as \verb+X+ 
  instead of \verb+-+;
\item \verb+std-symmetry+ (which may be abbreviated `\verb+std-sym+')
  instructs \methsearch\ to only use the ampersand notation for palindromic
  place notations\index{symmetry!palindromic} when the symmetry point is the
  final change as in Plain Bob;
\item \verb+no-symmetry+ (which may be abbreviated `\verb+no-sym+')
  instructs \methsearch\ to never use the ampersand notation for palindromic
  place notations; and 
\item \verb+plus-prefix+ (which may be abbreviated `\verb+plus+' or
  `\verb-+-') instructs \methsearch\ to add a \verb-+- prefix to any block
  which does not have an \verb+&+ prefix.
\end{itemize}
