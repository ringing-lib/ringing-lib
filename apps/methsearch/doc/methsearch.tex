...
For example, the \ttcmdidx{touchsearch} program could be invoked by a 
\verb+--require+ option to check that a touch of the method is actually 
possible:
\begin{Verbatim}[commandchars=@~~]
--require='$(touchsearch -b8 -r -l720 @hfill\
...
determine whether the method is a seconds place one, in which a \verb+234+ 
...
attempts to adopt a pragmatic solution to such situations.
...
\footnote{Some programs will accept permissive formats: for example
...
Charles Middleton's\index{Middleton, Charles!Cambridge} 5600 Cambridge Surprise Major can be defined as 
follows:
\begin{Verbatim}
8 bells
