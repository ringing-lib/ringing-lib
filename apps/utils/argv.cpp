...
// The vast majority of this code is taken from the libiberty library in 
// GNU binutils, version 2.21 [see http://www.gnu.org/software/binutils/
// for details], and is under the following copyright:
//
...
int expandargv (arg_parser& ap, int *argcp, char ***argvp);
void freeargv (arg_parser const& ap, char **vector);
char **buildargv (arg_parser& ap, const char *input);
...
static char **
dupargv (arg_parser const& ap, char **argv)
{
  int argc;
  char **copy;
...
char **buildargv (arg_parser const& ap, const char *input)
{
  char *arg;
  char *copybuf;
  int squote = 0;
...
@deftypefn Extension int expandargv (int *@var{argcp}, char ***@var{argvp})
