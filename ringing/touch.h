...
class touch_node {
public:
  typedef list<touch_node>::iterator pointer;
  typedef list<pair<int, pointer> > child_list;
...
public:
  void push_back(pointer p, int i = 1) {
    children.push_back(pair<int, pointer>(i, p));
  }
};
...
public:
  typedef list<touch_node>::iterator pointer;
  typedef list<touch_node>::const_iterator const_pointer;
...
public:
  ...
  class iterator : public forward_iterator<change, int> {
  private:
    struct place {
      int count;
      touch_node::child_list::const_iterator curr, end;
      place(const touch_node& n) : count(0) {
